...
| Pre-computed activity summaries | `BASE_ACTIVITY_SUMMARY` (device-agnostic, no HUAMI_ prefix) | `duration_s`, `base_longitude_raw`/`base_latitude_raw` (RAW, unscaled - see below), `base_altitude_m` (pre-scaled to meters by Gadgetbridge's own code before it reaches this column); tags `name` (`"Unset"` if null), `activity_kind_summary` (raw numeric, NOT assumed to share `activity_kind`'s code space above - a different table, unconfirmed either way), `sample_type: "activity_summary"`. Genuinely sparse in practice (see FIELD_RESEARCH.md) - populated only for deliberately-started workouts, not ambient daily movement. `RAW_SUMMARY_DATA` (Zepp OS devices - the richer per-workout breakdown) is now decoded via `flatten_workout_summary()`, confirmed directly against Gadgetbridge's own real source (`proto/huami.proto` + `ZeppOsActivitySummaryParser.java`, 2026-09) rather than a guessed shape - see that function's own docstring for the full field list (HR/HR zones, training load/effect, VO2max, altitude/elevation, cadence, swimming data, movement evaluation) and every confirmed scaling factor. `SUMMARY_DATA` (the plaintext JSON alternative) remains unextracted - independently confirmed Gadgetbridge strips it before writing to the DB to save space, so it's expected empty/null in practice. `base_longitude_raw`/`base_latitude_raw` are intentionally UNscaled - the `.proto`'s own inline comment claims a `/6000000` conversion to real coordinates, but Gadgetbridge's real parsing code never applies it (stores raw), so this parser doesn't invent that transformation either. |
| Per-workout detail (per-sample GPS/HR/cadence/etc.) | `BASE_ACTIVITY_SUMMARY.RAW_DETAILS_PATH` correlated against Gadgetbridge's own "Auto export GPX/FIT tracks" automation output (a SEPARATE file, NOT inside `Gadgetbridge.db` - see FIELD_RESEARCH.md for the full reachability investigation) | `sample_type: "workout_detail"`, one InfluxDB point PER SAMPLE (not per workout - typically one every 1-2 seconds during a workout), tagged with `workout_start_time` (the parent workout's own `BASE_ACTIVITY_SUMMARY.START_TIME`, linking each sample back to its own summary point) and `source_format` (`"fit"` or `"gpx"`, whichever was actually used). Fields vary per sample (a real FIT record can carry only some of these depending on what the device/activity type actually reports that instant): `hr`, `cadence_rpm`, `distance_m`, `altitude_m` (FIT's own higher-precision `enhanced_altitude` preferred over plain `altitude` when both are present), `speed_mps` (same enhanced-preferred rule), `latitude`/`longitude` (FIT's own "semicircles" coordinate encoding converted to real degrees: `degrees = semicircles * (180 / 2**31)`), `temperature_c`, `power_watts`, `step_length_mm`, `grade_percent`. Source priority, confirmed and explicit, not a guess: FIT preferred when available (confirmed to work for GPS AND non-GPS activities alike, and to carry HR/cadence/power alongside position - genuinely richer than GPX), GPX as a fallback (only exists for GPS-tracked activities, and Gadgetbridge's own GPX export has not been confirmed to embed HR/cadence via a `<gpxtpx:TrackPointExtension>` the way some other GPX generators do - only lat/lon/elevation/time extracted from it), and if NEITHER export is found for a given workout (not configured at all, or the workout predates enabling the automation - both real, confirmed-observed situations), no per-sample data is written but the workout's own SUMMARY point (from `RAW_SUMMARY_DATA` above) is unaffected - this is a purely additive enrichment layered on top, never required for the summary itself. Requires `EXPORT_TRACKS_PATH` to be explicitly configured (no default - silently does nothing, not even a log line, when unset, since there's no way to tell "not configured" apart from "configured wrong" without the person saying which). Each workout's own export is downloaded and parsed AT MOST ONCE ever (queries InfluxDB directly for which `workout_start_time` values already have detail points, before attempting any WebDAV list/download for that cycle) - not re-fetched on every sync cycle for as long as it stays within the checkpoint window. |
